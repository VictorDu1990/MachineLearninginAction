...
def testRbf(k1 = 1.3):
	...
	Parameters:
		k1 - Representation of arrival rate using Gauss kernel function
	Returns:
	    none
	'''
	dataArr,labelArr = loadDataSet('testSetRBF.txt')						#loadDataSet
	b,alphas = smoP(dataArr, labelArr, 200, 0.0001, 100, ('rbf', k1))		#callable b and alphas
	datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()
	svInd = np.nonzero(alphas.A > 0)[0]										#get the Support Vector
	sVs = datMat[svInd] 													
	labelSV = labelMat[svInd];
	print("number of Support Vector:%d" % np.shape(sVs)[0])
	m,n = np.shape(datMat)
	errorCount = 0
	for i in range(m):
		kernelEval = kernelTrans(sVs,datMat[i,:],('rbf', k1))				#calculate the kernel of each point
		predict = kernelEval.T * np.multiply(labelSV,alphas[svInd]) + b 	#according to Support Vetor to get Separated hyperplanes ,get the classification
		if np.sign(predict) != np.sign(labelArr[i]): errorCount += 1		#Returns the positive and negative symbols of each element in the array, expressed in 1 and - 1, and counts the number of errors
	print("traing sets error rate: %.2f%%" % ((float(errorCount)/m)*100)) 	#print the error rate
	dataArr,labelArr = loadDataSet('testSetRBF2.txt') 						#load the testsets
	errorCount = 0
	datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose() 		
	m,n = np.shape(datMat)
	for i in range(m):                                                      
